...
class CourseQueries(graphene.ObjectType):
    ...
    @login_required
    def resolve_courses(self, info, program:str=None, year:int=None, sem:int=None):
        try:
            courses = Course.objects.all()
            if program is not None:
                courses = courses.objects(curriculum_id__program=program)
            if year is not None:
                courses = courses.filter(curriculum_id__year=year)
            if sem is not None:
                courses = courses.filter(curriculum_id__sem=sem)
            print("courses:", courses)
            return courses
        except Course.DoesNotExist as e:
            raise APIException("Courses does not exist", code="INVALID_INPUT")
