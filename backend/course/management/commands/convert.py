...
for file in files:
    ...
    for key in temp:
        if key.startswith('+'):
            output['extra'][key[1:-6]] = [{'code': line[:8], 'name': line[8:-6] ,'L': line[-5], 'T': line[-4], 'P': line[-3], 'C': line[-1]} for line in temp[key]]
        else:
            output['semsters'][int(key[-2:])] = {
                'courses': [{'code': line[:7], 'name': line[8:-6] ,'L': line[-5], 'T': line[-4], 'P': line[-3], 'C': line[-1]} for line in temp[key] if not line.startswith('+')],
