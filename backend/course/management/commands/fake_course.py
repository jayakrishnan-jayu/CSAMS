...
def create_fake_courses():
    ...
    for file in files:
        with open(f'course/management/commands/{file}.json', 'r') as courseFile:
            ...
            program = Program.objects.get(name=output['program'])
            year = int(output['year'])
            curriculum, new = Curriculum.objects.get_or_create(program=program, year=year)
            semsters = output['semesters']
            for year_index in range(program.year):
                for sem in semsters:
                    batch, new = Batch.objects.get_or_create(curriculum=curriculum, year=year+year_index, sem=int(sem['sem']))
                    courses = [Course.objects.get_or_create(code=c['code'], name=c['name'], batch=batch, l=c['L'], t=c['T'], p=c['P'], credit=c['C'], hours=0)[0] for c in sem['courses']]
                    ...
                    #     print(extra)
                        ...
                        for i in output['extra']:
                            if extra in i['name']:
                                extraCourses += [ExtraCourse.objects.get_or_create(code=ec['code'].strip(),name=ec['name'], l=ec['L'], t=ec['T'], p=ec['P'], credit=ec['C'], hours=0, course_type=course_type, is_elective=is_elective)[0] for ec in i['courses']]
                                break
            #         print("randomly")
                    ...
                    for bce in batch.extras:
                        ...
                        while True:
                            ...
                            print(selected_ec.count(), bce.count)
